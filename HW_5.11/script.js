...
function GenDefs(){
    document.getElementById('Def').children[0].innerHTML += '<form id="CF"><input type="text" id="CT" placeholder="Table_Width_Here!" oninput="SwitchButtonText()"><select oninput="SwitchButtonText()"><option disabled selected>Border</option><option>hidden</option><option>dotted</option><option>dashed</option><option>solid</option></select><input id="CB" type="button" onclick="SwitchTableWidth()" value="Apply_Width_And_Border!"></form><br>';
    document.getElementById('Def').children[1].innerHTML += '<input type="button" onclick="Gen_Header()" value="Create_Header!"><br><br>';
    document.getElementById('Def').children[2].innerHTML += '<form><input type="text" placeholder="Row_To_Delete!" id="DelRow"><input type="button" onclick="Del_Row()" value="Delete_It!"></form><br>';
    document.getElementById('Def').children[3].innerHTML += '<input type="button" onclick="RandTheTable()" value="Magick!"><br><br>';
...
function Del_Row() {
    ...
    } else{ alert("DO_NOT_TRY_TO_XSS_ME_SCRIPTKIDDIE!"); }
    return;
}
